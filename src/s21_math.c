...
long double s21_pow(double base, double exp) {
  if ((base * 0 == 0) && (base < 0) && (exp * 0 == 0) &&
      (s21_floor(exp) != exp))
    return S21_NAN;
  if ((base == 1) || (exp == 0)) return 1.0;
  if ((base != base) || (exp != exp)) return S21_NAN;
  if ((base == 0) && (exp > 0) &&
      !((s21_fmod(s21_fabs(exp), 1.0) < S21_EPS) &&
        (s21_fmod(s21_fabs(exp), 2.0) > S21_EPS)))
    return 0;
  ...
  if ((base * 0 != 0) && (base < 0) &&
      (s21_fmod(s21_fabs(exp), 2.0) > S21_EPS) &&
      (s21_fmod(s21_fabs(exp), 1.0) < S21_EPS) && (exp < 0))
    return -0;
  if ((base * 0 != 0) && (base < 0) &&
      !((s21_fmod(s21_fabs(exp), 2.0) > S21_EPS) &&
        (s21_fmod(s21_fabs(exp), 1.0) < S21_EPS)) &&
...
long double s21_exp(
    double x) {  // exp(x) = 1 + x + x^2/2! + x^3/3! + x^4/4! + ... + x^n/n!
  if (x != x) return S21_NAN;
